...
LXQtGroupPopup::LXQtGroupPopup(LXQtTaskGroup *group):
    ...
    mGroup(group)
{
    qDebug() << "[LXQtGroupPopup::LXQtGroupPopup] New LXQtGroupPopup";
    Q_ASSERT(group);
    setAcceptDrops(true);
...
{
    qDebug() << "[LXQtGroupPopup::closeTimerSlot] start";
    ...
    if (!button_has_dnd_hover)
        close();
    qDebug() << "[LXQtGroupPopup::closeTimerSlot] end";
}
