...
************************************************/
LXQtTaskButton::LXQtTaskButton(const uint window, DBusAppListener *dbusAppListener, LXQtTaskBar2 * taskbar, QWidget *parent) :
    QToolButton(parent),
    mWindow(window),
    mUrgencyHint(false),
    mOrigin(Qt::TopLeftCorner),
    mParentTaskBar2(taskbar),
    mPlugin(mParentTaskBar2->plugin()),
    ...
    mWheelTimer(new QTimer(this))
{
    ...
    //setUrgencyHint(NETWinInfo(QX11Info::connection(), mWindow, QX11Info::appRootWindow(), NET::Properties{}, NET::WM2Urgency).urgency() || KWindowInfo{mWindow, NET::WMState}.hasState(NET::DemandsAttention));

    connect(LXQt::Settings::globalSettings(), &LXQt::GlobalSettings::iconThemeChanged, this, &LXQtTaskButton::updateIcon);
    connect(mParentTaskBar2,                   &LXQtTaskBar2::iconByClassChanged,        this, &LXQtTaskButton::updateIcon);
}
...
{
    QString title = mDbusAppListener->getWindowTitle(mWindow);
...
{
    QString windowAppId = mDbusAppListener->getWindowAppId(mWindow);
    QIcon ico = XdgIcon::fromTheme(windowAppId.toLower());
    if(ico.isNull())
        ico = QIcon::fromTheme(windowAppId);
    if(ico.isNull())
        qDebug() << "[LXQtTaskButton::updateIcon] Icon name " << windowAppId;
...
void LXQtTaskButton::mousePressEvent(QMouseEvent* event)
{
    ...
    else if (Qt::MiddleButton == b && parentTaskBar2()->closeOnMiddleClick())
        closeApplication();
...
void LXQtTaskButton::wheelEvent(QWheelEvent* event)
{
    ...
    if (mParentTaskBar2->wheelEventsAction() < 2 || mParentTaskBar2->wheelEventsAction() > 5)
    ...
    if (mWheelDelta < mParentTaskBar2->wheelDeltaThreshold())
        return QToolButton::wheelEvent(event);
    else
    {
        mWheelDelta = 0;
    ...
    if (mParentTaskBar2->wheelEventsAction() == 4)
    {
        moveApplicationToPrevNextDesktop(D < 0);
    }
    else if (mParentTaskBar2->wheelEventsAction() == 5)
    {
        moveApplicationToPrevNextDesktop(D > 0);
    }
    else
    {
        if (mParentTaskBar2->wheelEventsAction() == 3)
            D *= -1;
        if (D < 0)
            raiseApplication();
        else if (D > 0)
...
void LXQtTaskButton::mouseMoveEvent(QMouseEvent* event)
{
    ...
    drag->setPixmap(img);
    switch (parentTaskBar2()->panel()->position())
    {
...
{
    mDbusAppListener->windowSetActive(mWindow);
 ...
 ************************************************/
void LXQtTaskButton::minimizeApplication(bool changeState)
{
  if (changeState || QStringLiteral("MINIMIZED") != mDbusAppListener->getWindowState(mWindow)) 
    mDbusAppListener->windowChangeMinimized(mWindow);
}

/************************************************

 ************************************************/
void LXQtTaskButton::maximizeApplication(bool changeState)
{
    if (changeState || QStringLiteral("MAXIMIZED") != mDbusAppListener->getWindowState(mWindow)) {
      mDbusAppListener->windowChangeMaximized(mWindow);
    }
}
 ...
 ************************************************/
void LXQtTaskButton::fullScreenApplication(bool changeState)
{
  if (changeState || QStringLiteral("FULLSCREEN") != mDbusAppListener->getWindowState(mWindow)) 
    mDbusAppListener->windowChangeFullscreen(mWindow);
}

/************************************************

 ************************************************/
void LXQtTaskButton::restoreApplication()
{
  QString state = mDbusAppListener->getWindowState(mWindow);
    if (QStringLiteral("MAXIMIZED") == state) {
      mDbusAppListener->windowChangeMaximized(mWindow);
    } else if(QStringLiteral("MINIMIZED") == state) {
      mDbusAppListener->windowChangeMinimized(mWindow);
    } else if(QStringLiteral("FULLSCREEN") == state) {
      mDbusAppListener->windowChangeFullscreen(mWindow);
    }
}
...
{
    mDbusAppListener->windowClose(mWindow);
}
...
void LXQtTaskButton::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    */
    ...
    connect(a, &QAction::triggered, this, &LXQtTaskButton::restoreApplication);
    ...
    a = menu->addAction(tr("&Full screen"));
    connect(a, &QAction::triggered, this, &LXQtTaskButton::fullScreenApplication);
    ...
    menu->setGeometry(mParentTaskBar2->panel()->calculatePopupWindowPos(mapToGlobal(event->pos()), menu->sizeHint()));
...
{
    return QApplication::desktop()->screenGeometry(parentTaskBar2()).intersects(KWindowInfo(mWindow, NET::WMFrameExtents).frameGeometry());
}
