...
public:
    LXQtTaskGroup(const QString & groupName, uint window, DBusAppListener *dbusAppListener, LXQtTaskBar2 * parent);
    ...
    LXQtTaskButton * addWindow(uint id);
...
public slots:
    void onWindowRemoved(uint window);
...
private slots:
    ...
    void onActiveWindowChanged(uint window);
    void onDesktopChanged(int number);
    void onWindowTitleChanged(uint window, QString title);
