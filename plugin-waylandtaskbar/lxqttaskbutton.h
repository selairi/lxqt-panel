...
public:
    explicit LXQtTaskButton(const uint window, DBusAppListener *dbusAppListener, LXQtTaskBar2 * taskBar, QWidget *parent = nullptr);
    virtual ~LXQtTaskButton();

    bool isApplicationHidden() const;
    bool isApplicationActive() const;
    uint windowId() const { return mWindow; }
    ...
    LXQtTaskBar2 * parentTaskBar2() const {return mParentTaskBar2;}
...
public slots:
    ...
    void minimizeApplication(bool changeState = false);
    void maximizeApplication(bool changeState = false);
    void fullScreenApplication(bool changeState = false);
...
protected:
    ...
    void setWindowId(uint wid) {mWindow = wid;}
...
typedef QHash<uint,LXQtTaskButton*> LXQtTaskButtonHash;
